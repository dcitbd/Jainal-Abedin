/* =====================================================
   ADMIN TEMPLATES PAGE JAVASCRIPT (FIXED & SYNCED)
...
(() => {
  ...
  const normalise = (item, index) => ({
    ...
    tags: Array.isArray(item.tags) ? item.tags : String(item.tags || item.technology || '').split(',').map(tag => tag.trim()).filter(Boolean),
  ...
  function readTemplates() { 
    try { 
      ...
      return Array.isArray(saved) ? saved.map(normalise) : []; 
    } catch { 
      return []; 
    } 
  }
  ...
  function updateCategoryOptions() { 
    ...
    const categories = [...new Set(templates.map(item => item.category).filter(Boolean))].sort(); 
  ...
  function filteredTemplates() { 
    ...
    return templates.filter(item => (!term || [item.title, item.category, item.description, ...item.tags].join(' ').toLowerCase().includes(term)) && (!category.value || item.category === category.value) && (!status.value || item.status === status.value)); 
  }
  ...
  function render() { 
    ...
    const active = Boolean(search.value || category.value || status.value); 
    clear.classList.toggle('visible', active); 
    $('#totalTemplates').textContent = templates.length; 
    $('#publishedTemplates').textContent = templates.filter(item => item.status === 'published').length; 
    $('#draftTemplates').textContent = templates.filter(item => item.status === 'draft').length; 
    result.textContent = templates.length ? `${list.length} of ${templates.length} template${templates.length === 1 ? '' : 's'} shown` : 'Your saved template collection'; 
    ...
    grid.innerHTML = list.map(item => `
      <article class="template-card">
        ...
        <div class="template-card-body">
          ...
          <div class="template-tags">${item.tags.slice(0,3).map(tag => `<span>${escapeHtml(tag)}</span>`).join('')}</div>
          <div class="template-actions">
            ...
            <button class="template-action" type="button" data-action="view" data-id="${escapeHtml(item.id)}" title="View public template"><i class="fa-solid fa-arrow-up-right-from-square"></i></button>
  ...
  function toast(message, icon = 'fa-circle-check') { 
    ...
    window.setTimeout(() => $('#adminToast').classList.remove('show'), 2800); 
  }
  ...
  function openPreview(item) { 
    ...
    $('#previewContent').innerHTML = `
      ...
      <div class="preview-modal-body">
        ...
        <p>${escapeHtml(item.description)}</p>
        ${item.tags.length ? `<div class="template-tags" style="margin-top:10px;">${item.tags.map(tag => `<span>${escapeHtml(tag)}</span>`).join('')}</div>` : ''}
  ...
  function remove(id) { 
    ...
    const raw = readTemplates(); 
    const retained = raw.filter((entry, index) => String(normalise(entry, index).id) !== String(id)); 
  ...
  grid.addEventListener('click', event => { 
    ...
    if (!button) return; 
    const item = find(button.dataset.id); 
    if (!item) return; 
    ...
    if (button.dataset.action === 'delete') remove(item.id); 
  });
